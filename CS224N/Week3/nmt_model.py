...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        X = self.post_embed_cnn(X)  # (b, e, src_len) (padding='same' preserves length)
        ...
        enc_hiddens, _ = pad_packed_sequence(packed_enc_hiddens)  # (src_len, b, 2h)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # (b, src_len, 2h)
        last_hidden_cat = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # (b, 2h)
        last_cell_cat = torch.cat((last_cell[0], last_cell[1]), dim=1)  # (b, 2h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #enc_hiddens_proj: (b, s, h)
        #dec_hidden: (b, h) -> squeeze -> (b, h, 1)
        #(b, s, h) @ (b, h, 1) -> (b, s, 1) -> (b, s)  
        ### END YOUR CODE
